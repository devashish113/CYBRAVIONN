...
import { 
  ...
  Building2, Heart, Cloud, CheckCircle2 
} from 'lucide-react';
...
const caseStudies = [
  {
    ...
    results: [
      { metric: "94%", label: "Reduction in exploitable attack surface" },
...
export const CaseStudies: React.FC = () => {
  ...
  return (
    <section id="case-studies" className="py-24 md:py-32 px-6 md:px-12 lg:px-20 relative z-10 bg-stone-950/60 border-t border-stone-900">
              ...
              <TiltCard3D key={cs.id} glowColor={cs.glow}>
                <div className="flex flex-col h-full justify-between">
                  <div>
                    ...
                    {/* Metric Highlights */}
                    <div className="space-y-3 py-4 border-y border-stone-800/80 mb-6">
                      {cs.results.map((r, idx) => (
                        <div key={idx} className="flex items-center justify-between">
                          <span className="text-xs text-stone-400 font-light">{r.label}</span>
                          <span className="font-bold text-sm text-blue-400">{r.metric}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-3 px-4 rounded-xl bg-stone-900 hover:bg-blue-500/20 border border-stone-800 hover:border-blue-500/40 text-xs text-stone-300 hover:text-white font-semibold transition-all flex items-center justify-center gap-2 cursor-pointer"
                  >
            ...
            {caseStudies
              ...
              .map((c) => (
                <motion.div
                  ...
                  className="bg-stone-950 border border-blue-500/30 rounded-3xl p-6 sm:p-8 max-w-2xl w-full shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_30px_rgba(59,130,246,0.15)] relative max-h-[90vh] overflow-y-auto"
                >
                  <button
                    onClick={() => setExpandedCase(null)}
                    className="absolute top-6 right-6 p-2 text-stone-400 hover:text-white rounded-full bg-stone-900 border border-stone-800 cursor-pointer"
                  >
                    <X size={18} />
                  </button>

                  <span className="text-xs uppercase text-blue-400 tracking-wider block mb-2 font-semibold">
                  ...
                  <div className="space-y-6 text-sm">
                    ...
                    <div>
                      <h4 className="font-semibold text-blue-400 text-xs uppercase tracking-wider mb-2">
                    ...
                    <div>
                      <h4 className="font-semibold text-orange-400 text-xs uppercase tracking-wider mb-3">
                        Measurable Impact
                      </h4>
                      <div className="grid grid-cols-3 gap-3">
                        {c.results.map((r, i) => (
                          <div key={i} className="p-3.5 bg-stone-900/80 rounded-2xl border border-stone-800 text-center">
                            <div className="font-bold text-lg text-blue-400 mb-1">{r.metric}</div>
                    ...
                    <div className="p-4 rounded-2xl bg-blue-950/20 border border-blue-500/20 italic text-stone-300 text-xs leading-relaxed">
                      "{c.testimonial}"
                      <span className="block not-italic text-[10px] text-blue-400 mt-2 font-semibold">— {c.role}</span>
                    </div>
                  </div>
                </motion.div>
              ))}
